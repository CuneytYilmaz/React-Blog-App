...
try {
  var localWebpackDevServer = require.resolve(
    path.join(
      process.cwd(),
      "node_modules",
      "webpack-dev-server",
...
var optimist = require("optimist")
  ...
  .describe("lazy")
  .boolean("stdin")
  .describe("stdin", "close when stdin ends")
  .boolean("info")
  .describe("info")
  .default("info", true)
  .boolean("quiet")
  .describe("quiet")
  .boolean("inline")
  .describe("inline", "Inline the webpack-dev-server logic into the bundle.")
  .boolean("https")
  .describe("https")
  .string("key")
  .describe("key", "Path to a SSL key.")
  .string("cert")
  .describe("cert", "Path to a SSL certificate.")
  .string("cacert")
  .describe("cacert", "Path to a SSL CA certificate.")
  .string("pfx")
  .describe("pfx", "Path to a SSL pfx file.")
  .string("pfx-passphrase")
  .describe("pfx-passphrase", "Passphrase for pfx file.")
  .string("content-base")
  .describe("content-base", "A directory or URL to serve HTML content from.")
  .string("content-base-target")
  .describe("content-base-target", "Proxy requests to this target.")
  .boolean("history-api-fallback")
  .describe(
    "history-api-fallback",
    "Fallback to /index.html for Single Page Applications."
  )
  .string("client-log-level")
  .describe(
    "client-log-level",
    "Log level in the browser (info, warning, error or none)"
  )
  .default("client-log-level", "info")
  .boolean("compress")
  .describe("compress", "enable gzip compression")
  .boolean("open")
  .describe("open", "Open default browser")
  .describe("port", "The port")
  .default("port", 8084)
  .describe("public", "The public hostname/ip address of the server")
  .describe("host", "The hostname/ip address the server will bind to")
...
function processOptions(wpOpt) {
  ...
  if (argv.port !== 8080 || !options.port) options.port = argv.port;
  ...
  if (
    !options.watchDelay &&
    !options.watchOptions // TODO remove in next major version
  )
  ...
  if (argv["inline"]) options.inline = true;
  ...
  if (argv["open"]) options.open = true;
  ...
  if (options.inline) {
    ...
    if (options.hot) devClient.push("webpack/hot/dev-server");
    [].concat(wpOpt).forEach(function(wpOpt) {
  ...
  new Server(
    ...
    if (!options.inline) uri += "webpack-dev-server/";
    ...
    else console.log("content is served from " + options.contentBase);
    if (options.historyApiFallback)
      console.log(
        "404s will fallback to %s",
